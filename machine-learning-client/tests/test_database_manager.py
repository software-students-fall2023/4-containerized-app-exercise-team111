...
def test_save_prediction(mock_db_manager):
    ...
    result_id = mock_db_manager.save_prediction(test_data['image_path'], test_data['predictions'])
    assert result_id is not None

def test_get_results(mock_db_manager):
    mock_db_manager.save_prediction("tests/images/tiger_testImage.jpg", [{"label": "tiger", "probability": 0.9}])
